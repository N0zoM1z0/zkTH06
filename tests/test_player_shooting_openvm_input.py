#!/usr/bin/env python3
"""Validate deterministic construction of the shooting OpenVM workload."""
...
VECTOR_MANIFEST = ROOT / "evidence" / "player-shooting-002677-2000-v1.json"
BUILDER = ROOT / "tools" / "build_player_shooting_openvm_input.py"
VECTOR_HEADER_BYTES = 192
VECTOR_RECORD_BYTES = 32
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="zkth06-openvm-shooting-input-") as directory:
        ...
        assert report["expected_final_spawn_call_count"] == 67
        final_state = FINAL_STATE.pack(
            ...
            report["expected_final_previous_frame_input"],
            report["expected_final_fire_timer_previous"],
            report["expected_final_fire_timer_current"],
            report["expected_final_spawn_call_count"],
        )
    ...
    print("validated deterministic shooting OpenVM input and statement construction")
    return 0
