...
executions of the exact \code{\_\_ftol2} body against dyadic truncation.  This
surface is now indexed by a hash-bound ledger of 244 comparison and 77 helper
addresses.  A bit-mask re-audit records 75 full-EAX and two AL-only helper
observers while explicitly retaining unresolved control-flow obligations.
This remains counterexample search and bookkeeping: startup state,
load/remainder behavior, reachable helper ranges, code correspondence, and
transcendental microarchitecture are not yet proved.  The fidelity design
therefore binds operations to original addresses and store widths instead of
assuming source-level host arithmetic is exact.
