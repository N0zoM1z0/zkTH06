...
experiments select a backend only for a retail-anchored player-motion slice.
The first proof hash-binds per-frame environment records; the second derives
those records inside a fail-closed enclosing player-state profile and commits
only fixed configuration, replay masks, and the reached endpoint.  Neither
