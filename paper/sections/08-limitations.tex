...
\begin{itemize}
  ...
  bullet motion, ANM reclamation, or collision.  A shorter enclosing layer now
  derives 206 consecutive rank-1 transitions from an empty pool through motion,
  bounds reclamation, fixed ANM/timer behavior, and allocation, but deliberately
  stops before the first Enemy collision at frame 208.  It does not cover ranks 4--5,
  the other shot routes, laser paths, or bullet collision.  Its dyadic rounding
  ...
  data.  The lifecycle extension removes that witness boundary only for its
  206-transition collision-free prefix; it does not derive the frame-208
  Enemy/ECL write or prove ANM-data and writer correspondence.
