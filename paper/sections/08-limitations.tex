...
\begin{itemize}
  ...
  probe now agrees with the Linux reference for 2,000 frames and a 50-field
  semantic projection on one replay.  An enlarged Enemy/ECL/Item/RNG/bullet
  projection also agrees for 1,200 frames and crosses the first seven live
  Enemy bullets.  Collided-bullet sprite z, the Item offscreen-indicator flag,
  and ANM sprite load handles are explicit presentation-only exclusions with
  next-read arguments, not raw equality.  The probe does not yet export the eleven canonical
  ...
  player-position transition, enclosing life-timer profile, and shooting
  cadence match 1,999
  ...
  and it does not model hitbox/orb/animation side effects.  The cadence layer
  stops at \code{SpawnBullets} dispatch.  A separate local callback layer does
  model all 80 slot states and Reimu-A ranks 1--3 initialized geometry, but its
  1,590 pre-states are independently observed rather than derived through
  bullet motion, ANM reclamation, or collision.  One enclosing layer derives
  206 consecutive rank-1 transitions from an empty pool through motion, bounds
  reclamation, fixed ANM/timer behavior, and allocation.  A successor composes
  five fixed early enemies and derives the first collision, Enemy death, target,
  and score at frame 208.  A successor retains all collided slots and derives
  all five first-wave deaths through frame 229, with a public commitment to
  every active bullet and Enemy projection.  The latest successor derives the
  death-drop cursors, first Item spawn/motion/collection, collision-ANM
  reclamation, and score/power/subrank feedback through frame 249.  A further
  successor derives the second Enemy group, ECL movement, effect RNG, two Item
  drops, and explicitly empty Enemy-bullet state through frame 350.  It does not
  cover ranks 4--5, the other shot routes,
  laser paths, live enemy bullets, later ECL paths, or Player death.  Its dyadic rounding
  ...
  The shooting extension additionally fixes the dialogue predicate false for
  this interval; the 2,000 observed false values do not prove dialogue
  noninterference or writer completeness.
  The bullet-spawn extension supplies complete pre-call slot arrays as private
  data.  The lifecycle extension removes that witness boundary for its
  206-transition collision-free prefix, and the early-gameplay extension
  derives the frame-208 Enemy/ECL write.  The latter still uses a finite
  ECL-time-indexed x87 result table; the first-wave and Item successors add
  finite collision-ANM and Item behavior but do not decode those scripts
  directly.  The second-wave successor retains shared RNG/effects and the
  empty Enemy-bullet pool through frame 350, but does not yet implement the
  first live bullets observed at frame 1180 or prove omitted-shooting
  noninterference, transcendental refinement, or complete writer correspondence.
  ...
  read later.  The collided Player-bullet sprite-z write is now explicitly
  shown to be overwritten before the next relevant read in the selected
  lifecycle projection; every other removal still needs dependency analysis
