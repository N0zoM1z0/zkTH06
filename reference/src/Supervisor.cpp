...
ChainCallbackResult Supervisor::OnUpdate(Supervisor *s)
{
    ...
    if (g_HeadlessRuntime.enabled && g_HeadlessRuntime.inputError)
    {
...
ZunResult Supervisor::AddedCallback(Supervisor *s)
{
    ...
    g_HeadlessRuntime.actualSeed =
        g_HeadlessRuntime.seedProvided ? g_HeadlessRuntime.seed : (u16)std::time(NULL);
    g_Rng.Initialize(g_HeadlessRuntime.actualSeed);
...
ZunResult Supervisor::LoadConfig(const char *path)
{
    ...
    if (!g_HeadlessRuntime.enabled &&
        FileSystem::WriteDataToFile(path, &g_Supervisor.cfg, sizeof(GameConfiguration)) != 0)
    {
